      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ background: CINZA }}>
            {['Protocolo','Pet','Status','Data'].map(h => (
              <th key={h} style={{ padding: '12px 16px', fontWeight: 700, fontSize: '13px', color: '#888', textAlign: 'left', borderBottom: '1px solid #eee' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {items.map(s => (
            <tr key={s.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
              <td style={{ padding: '14px 16px' }}><span style={{ fontFamily: 'monospace', color: ROSA, fontWeight: 700 }}>{s.protocolo}</span></td>
              <td style={{ padding: '14px 16px', fontWeight: 600 }}>{s.petNome}</td>
              <td style={{ padding: '14px 16px' }}><Badge status={s.status} /></td>
              <td style={{ padding: '14px 16px', color: '#888', fontSize: '13px' }}>{formatData(s.criadoEm)}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ background: CINZA }}>
            {['Protocolo','Data','Horário','Motivo','Status'].map(h => (
              <th key={h} style={{ padding: '12px 16px', fontWeight: 700, fontSize: '13px', color: '#888', textAlign: 'left', borderBottom: '1px solid #eee' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {items.map(a => (
            <tr key={a.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
              <td style={{ padding: '14px 16px' }}><span style={{ fontFamily: 'monospace', color: ROSA, fontWeight: 700 }}>{a.protocolo}</span></td>
              <td style={{ padding: '14px 16px', fontWeight: 600 }}>{a.data}</td>
              <td style={{ padding: '14px 16px' }}>{a.horario}</td>
              <td style={{ padding: '14px 16px', color: '#666', fontSize: '13px' }}>{a.motivo || '—'}</td>
...
function AbaDoacoes({ uid }) {
  ...
  return (
    <>
      <div style={{ background: `linear-gradient(135deg, ${ROSA}, #7b1042)`, borderRadius: '14px', padding: '20px 24px', color: 'white', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
        <i className="bi bi-heart-fill" style={{ fontSize: '32px', opacity: 0.8 }}></i>
        <div>
          <div style={{ fontSize: '13px', opacity: 0.8 }}>Total doado</div>
          <div style={{ fontSize: '28px', fontWeight: 800 }}>R$ {total.toFixed(2)}</div>
        </div>
      </div>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: CINZA }}>
              {['Protocolo','Valor','Método','Status','Data'].map(h => (
                <th key={h} style={{ padding: '12px 16px', fontWeight: 700, fontSize: '13px', color: '#888', textAlign: 'left', borderBottom: '1px solid #eee' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {items.map(d => (
              <tr key={d.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                <td style={{ padding: '14px 16px' }}><span style={{ fontFamily: 'monospace', color: ROSA, fontWeight: 700 }}>{d.protocolo}</span></td>
                <td style={{ padding: '14px 16px', fontWeight: 700, color: ROSA }}>R$ {d.valor?.toFixed(2)}</td>
                <td style={{ padding: '14px 16px', color: '#666', fontSize: '13px', textTransform: 'capitalize' }}>{d.metodo === 'pix' ? 'PIX' : d.metodo === 'credito' ? 'Crédito' : 'Débito'}</td>
                <td style={{ padding: '14px 16px' }}><Badge status={d.status} /></td>
                <td style={{ padding: '14px 16px', color: '#888', fontSize: '13px' }}>{formatData(d.criadoEm)}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  if (carregando) {
    return (
      <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div className="spinner-border" style={{ color: ROSA, width: '48px', height: '48px' }}></div>
      </div>
    );
  }

  const abas = [
    { key: 'adocoes',      icon: 'bi-heart',           label: 'Solicitações de Adoção' },
    { key: 'agendamentos', icon: 'bi-calendar-check',  label: 'Agendamentos de Visita' },
    { key: 'doacoes',      icon: 'bi-cash-coin',       label: 'Minhas Doações' },
  ];
    ...
    <>
      ...
      <section style={{ backgroundColor: AMARELO, padding: '60px 0 80px', textAlign: 'center' }}>
        <div className="container">
          ...
          <p style={{ color: ROSA, marginTop: '8px', opacity: 0.8 }}>{usuario?.email}</p>
        </div>
      </section>
      ...
      <div className="container" style={{ maxWidth: '860px', marginBottom: '80px' }}>
        <div style={{ background: 'white', borderRadius: '20px', boxShadow: '0 10px 40px rgba(0,0,0,0.08)', marginTop: '-40px', position: 'relative', zIndex: 10, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid #f0f0f0', overflowX: 'auto' }}>
            {abas.map(a => (
              <button key={a.key} onClick={() => setAba(a.key)} style={{ flex: 1, minWidth: '160px', padding: '18px 16px', border: 'none', background: 'none', fontWeight: aba === a.key ? 700 : 500, fontSize: '14px', color: aba === a.key ? ROSA : '#888', borderBottom: `3px solid ${aba === a.key ? ROSA : 'transparent'}`, cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          ...
          {/* Rodapé */}
          <div style={{ borderTop: '1px solid #f0f0f0', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
            <Link to="/" className="btn btn-outline-secondary rounded-pill px-4" style={{ fontSize: '14px' }}>
              <i className="bi bi-house me-2"></i>Voltar ao Início
            </Link>
            <button onClick={sair} className="btn rounded-pill px-4" style={{ background: ROSA, color: 'white', fontWeight: 700, fontSize: '14px' }}>
